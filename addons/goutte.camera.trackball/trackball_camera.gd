...
# If release mouse button then immediately stop camera movement
export var mouse_release_stop = false
...
func handle_mouse_input(event):
	if (not mouse_move_mode) and (event is InputEventMouseButton):
		if event.pressed:
			...
			if mouse_release_stop:
				#upon mouse button release, stop moving the camera
